...
raw_data <- read_excel('"raw_data.xlsx',sheet = 3)

raw_data$dev_stage <- as.factor(raw_data$dev_stage)
raw_data$dev_stage <- fct_relevel(raw_data$dev_stage,"TS","GA","PM")
...
t.test(change_rate$florets_mean~change_rate$variety,paired=T)
